...
**وضع ملف قاعدة البيانات الحالي:** `PLUS NINETY PS/Project.Docs/Releases/R1/Database/DATABASE_DESIGN_R1.md`؛ أنشئ المجلد `Database` داخل `Releases/R1` إن لم يكن موجودًا. هو ملف تصميم لا SQL ولا Migration. **لا تغيّر تهجئة مجلدك `Project.Digrams` من تلقاء نفسك**؛ استخدم الاسم الموجود كما أرسلته، حتى إن كان المقصود لغويًا Diagrams.
...
**User says all requested diagrams have been drawn.** Keep `.drawio`/PlantUML sources. Do not recreate Use Case, Activity, Sequence, State, C4, Deployment, ERD or DFD solely for document count. Update **only impacted diagrams** after design decisions or for readable output.
...
`Project.Docs/Releases/R1/Database/DATABASE_DESIGN_R1.md`, Section 01, already proposes ownership for 18 logical entities; its **TBD/Proposed is not accepted automatically**. Design the **local SQLite and central SQL Server models explicitly**, even if many C# domain types are shared. Required decisions and checks:
...
| `Project.Docs/Releases/R1/R1_OPERATIONAL_MVP.md` أو نسخة `(1)` | `3. Release 1 Scope Summary` + `Cashier/Manager permissions`, `Invoices`, `Acceptance`, `Project Flow` | `Allowed predefined discounts` و`Manager-authorized manual discounts` قديمان، وتظهر خصومات في صلاحيات وخطط/اختبارات متفرقة. | احذف **خصومات R1 وحدها** من النطاق والمخططات ومتطلبات التنفيذ والاختبار. احتفظ بـ**Manager/Owner protected Invoice Cancel/Void** منفصلًا ولا تحذفه خطأً باعتباره discount override. مسار التشغيل نفسه Local-first Online/Offline. |
...
| `Project.Docs/Releases/R1/R1_ACCEPTANCE.md` (أينما يوجد) | الصفان **`R1-15` و`R1-16`** تحديدًا، و`R1-13`/`R1-14`, جدول milestone | R1-15 = `Allowed predefined discounts` وR1-16 = `Manual discount`; يمثلان ميزات لم تعد R1. | غيّر حالة `R1-15`/`R1-16` إلى **Removed from R1 scope — superseded** مع إحالة للقرار بدل **إعادة استخدام ID** لو مستندات أخرى تشير له؛ أزل اختبارات الخصم من R1. راجع `R1-13/14` للفوترة النقدية والإلغاء المحمي وعدم تكرار الدفع. |
...
| `Project.Docs/Requirements/INFORMATION_ARCHITECTURE_R1.md` أو `Project.Docs/Releases/R1/UX_UI/INFORMATION_ARCHITECTURE_R1.md` | Cashier nav, Management children, `There is no normal R1 navigation` | المصطلح `Discount Management` يظهر ضمن **قائمة المنع بالفعل**، وليس خطأ يتطلب حذفه؛ انتبه للـnegation. | اترك قسم الاستبعاد؛ التحقق من أن Cashier لا تظهر له management/reports وأن Employees management منفصل عن Shift. |
| `Project.Docs/Releases/R1/UX_UI/UI_REVIEW_R1.md` | `Purpose`, `Final Active Screen Baseline`, `Final UI Approval Checklist` | نص `as if already applied` وعلامات `[x]` تعني الموافقة على **التصميم المقصود**؛ قد يفسرها Codex خطأً كأن PNG/واجهة WPF اتعدلت بالفعل. | أضف أعلى الملف: **Approved target UI spec, not an audit of actual ZIP/WPF. The supplied ZIP contains 37 EN + 37 AR including retired 36; screenshots remain older. `[x]` here is design acceptance only, not implemented. Frontend MUST implement all amendments in §12.** لا تغيّر السلوك المصحح في بقية الملف. |
| `Project.Docs/Releases/R1/UX_UI/DESIGN_SYSTEM_R1.md` | Color tokens/font specs + `Sources` | ألوان HEX وأسماء خطوط مُستنتجة من صور؛ لا يوجد إثبات أن كل token exact Figma metadata. | أضف **Reference palette approximated from screenshots unless exact original Figma tokens are inspected; preserve dark/burgundy/primary red and EN/AR parity, do not promise pixel-perfect verified font/hex.** لا تستخدم اختلاف طفيف في صورة قديمة لاستبدال مرجع Approved UX. |
| `Project.Docs/Releases/R1/UX_UI/UX_FLOWS_R1.md` و`RBAC_MATRIX_R1.md` | Shift, Dashboard→Start, Cancel, Settings, Back | الموافق عليه design لا يعني وُجد تنفيذ فعلي، وبالأخص Shift employee administration مقابل Employees CRUD. | ضع reference `§12` للمقارنة أثناء التنفيذ؛ إن ظهر تعارض مع نسخة قديمة، النص المعتمد: **Shift Start/End for selected employee by authorized Manager/Owner; handover active sessions; Employee Management separate.** |
