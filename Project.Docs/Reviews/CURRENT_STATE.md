...
**Status:** BE-00 backend foundation and BE-01 billable-time domain calculation exist. BE-01.5 configures a conventional controller-based ASP.NET Core Web API with development-only Swagger UI; no new business module was implemented. Persistence/database, sessions, financial modules, authentication, sync, and WPF frontend are not implemented. The wider R1 pre-code gate remains on HOLD.
...
| Tests/hosting/DR | Health, development Swagger, and billable-time tests pass; no deployment or restoration validated; provider/machine details not confirmed |
...
| Codex | BE-00, BE-01, and BE-01.5 API setup explicitly approved; BE-02 and later slices remain unauthorized |
